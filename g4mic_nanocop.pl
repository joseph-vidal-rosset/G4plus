...
show_banner :-
    ...
    write('  fof_to_prove(File).       FOF file -> prove/1 command'), nl,
    write('  fof_to_prove_run(File).   FOF file -> full LaTeX proofs'), nl,
    write('  get_fof_problem(Formula). G4+ formula -> FOF file'), nl,
...
output_proof_results(ProofIn, LogicType, _OriginalFormula) :-
    ...
    % search (see the GAMMA COMPARTMENTS section). Rendering (raw dump,
    % bussproofs, ND-tree, Fitch) is the only place that needs the
    % original flat-list Gamma>Delta sequent, so normalize once here
    % rather than in provable_at_level/3 -- most callers of
    % provable_at_level (g4mic_logic_level/2 and friends) never render a
    % proof at all, and paying this cost on every search would be pure
    % waste for them.
...
help :-
    ...
    write('  CONVERTERS:'), nl,
    write('    fof_to_prove(\'myfile.p\').        - get prove/1 command from FOF file'), nl,
    write('    fof_to_prove_run(\'myfile.p\').    - FOF file -> full LaTeX proofs'), nl,
    write('    get_fof_problem(p => p).         - get FOF file from G4+ formula'), nl,
...
render_sequent(Gamma > Delta, VarCounter, FinalCounter) :-
    ...
    % Gamma is already a plain list here: normalize_proof_gammas/2
    % flattens every compartmentalised g/8 sequent in the proof tree
    % back to a list at the provable_at_level/3 boundary, so nothing
    % downstream of proof search ever sees a compartment term.
...
% (see the GAMMA COMPARTMENTS section) two or more (_&_)=>_ candidates
% can appear in either order in the flattened list, and a plain "first
% match" select no longer reliably lands on the one this rule actually
% fired on.
