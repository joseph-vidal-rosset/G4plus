...
prove(Left <=> Right) :-
    ...
    validate_and_warn(Left <=> Right, _ValidatedFormula),
        ...
        ( Direction1Valid = true ->
            write('[OK] VALID in '), write(Logic1), write(' logic')
        ; write('[!]  FAILED')
        ), nl,
        write('Direction 2 ('), write(Right => Left), write('): '),
        ( Direction2Valid = true ->
            write('[OK] VALID in '), write(Logic2), write(' logic')
        ; write('[!]  FAILED')
        ), nl, nl,
        ...
        ; G4micResult = invalid, NanoCopResult = invalid ->
            write('[OK] Both provers agree: '), write('false'), nl
...
prove(Formula) :-
    ...
    ; provable_at_level([] > [F2], constructive, Proof) ->
        write('+---------------------------------------------------------+'), nl,
        write('              [OK] INTUITIONISTIC LOGIC                      '), nl,
        write('+---------------------------------------------------------+'), nl,
    ...
    ; G4micResult = valid, NanoCopResult = invalid ->
        nl,
        write('============================================================='), nl,
        write('[!]  CRITICAL DISAGREEMENT: g4mic=true, nanoCoP=false'), nl,
        write('============================================================='), nl,
        nl,
...
% Lv-> : Disjunction to implications
...
fitch_g4_proof(lor((Premisss > [_Goal]), SP1, SP2), Context, Scope, CurLine, NextLine, ResLine, VarIn, VarOut) :-
    ...
    ; member((B => #), Premisss) ->
        ...
        !
    ;
        fail  % DS not applicable, fall through to regular vE
    ).

% L-or: Disjunction elimination (regular case with full vE)
...
% Lexistsv : Combined existential-disjunction
...
% Lv-> (Left disjunction to conditional)
